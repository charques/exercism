...
class CipherTest extends FunSuite with Matchers {
  ...
  test("Substitution cipher - can encode 2") {
    Cipher(Some("abcdefghij")).encode("abcdefghij") should be ("acegikmoqs")
  }

  test("Substitution cipher - can decode x") {
    Cipher(Some("abcdefghij")).decode("acegikmoqs") should be ("abcdefghij")
  }
