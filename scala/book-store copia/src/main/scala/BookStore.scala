...
object BookStore {
  ...
  @tailrec
  def generateGroupsByGroupSize(groupSize: Int, list: List[Int], counter:Int): (List[Int], Int) = {
    val groupList = list.distinct.sliding(groupSize, groupSize).toList.filter(_.size == groupSize)
    groupList match {
      case Nil => (list, counter)
      case _ =>
        var listP = list
        var counterP = counter
        for (group <- groupList) {
          listP = listP.diff(group)
          counterP += 1
        }
        generateGroupsByGroupSize(groupSize, listP, counterP)
    }
  }
  ...
  def generateGroupsByGroupSizeList(groupSizeList: List[Int], booksList: List[Int], result: List[(Int, Int)]): List[(Int, Int)] = {
    groupSizeList match {
      ...
      case x :: tail =>
        val groupsTuple = generateGroupsByGroupSize(x, booksList, 0)
